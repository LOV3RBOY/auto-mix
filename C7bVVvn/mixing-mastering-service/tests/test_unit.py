...
def test_process_mixing_job_success(mock_fs):
    ...
    write_mock = patch.object(pedalboard.io, 'write').start()
    process_mixing_job(request) # re-run to check call
    write_mock.assert_called_once()
    patch.stopall() # clean up
...
def test_dsp_pipeline_instantiation(mocker):
    ...
    assert mocker.patch.object(pedalboard, 'Pedalboard').call_count >= 2
